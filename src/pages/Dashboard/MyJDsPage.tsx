...
export const MyJDsPage = ({ onNavigate, onNavigateToJD }: MyJDsPageProps) => {
  ...
  useEffect(() => {
    const fetchJDs = async () => {
      ...
      try {
        ...
        const jdsRef = collection(db, 'jds');
        const q = query(
          jdsRef, 
        ...
        console.log(`${jds.length}개의 JD를 불러왔습니다.`);
        setJdList(jds);
      } catch (error) {
        console.error('JD 목록 불러오기 실패:', error);
        // orderBy 인덱스 에러가 발생할 경우 정렬 없이 다시 시도
        try {
          const user = auth.currentUser;
          if (!user) return;
          
          const jdsRef = collection(db, 'jds');
          const q = query(jdsRef, where('userId', '==', user.uid));
          const querySnapshot = await getDocs(q);
          ...
          console.log(`${jds.length}개의 JD를 불러왔습니다 (정렬 없이).`);
          setJdList(jds);
        } catch (retryError) {
  ...
  const handleDelete = async (jdId: string, e: React.MouseEvent) => {
    ...
    try {
      ...
      await deleteDoc(doc(db, 'jds', jdId));
